...
// gate, AppShell mounts and the chat page can fire API calls while the
// redirect handshake is still being processed - before there's an account or
// a usable token - which is what was causing the 401 -> loginRedirect ->
...
export function Providers({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("bot-forbidden", handleForbidden);
    return () => window.removeEventListener("bot-forbidden", handleForbidden);
  }, []);
          ...
          {isForbidden && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
              <div className="mx-4 max-w-md rounded-lg bg-white dark:bg-card p-6 shadow-xl text-center">
                ...
                <h2 className="mb-2 text-xl font-bold text-navy dark:text-white">Access Denied</h2>
